...
from entities import Player, Enemy, Platform, Powerup, Obstacle
...
class Game:
    ...
    def update(self):
        if self.state == GameState.PLAYING:
            ...
            if result == "death" or result == "hit":
                ...
                self.lives += 1
                self.score += 200
            if self.player.rect.right >= const.LEVEL_WIDTH - 5:
    ...
    def _add_difficulty_enemies(self):
        ...
        if len(self.enemies) >= 25:
            return
        new_enemies = [
            (random.randint(200, const.LEVEL_WIDTH - 200), random.randint(300, 500), "fast"),
            (random.randint(200, const.LEVEL_WIDTH - 200), random.randint(300, 500), "jumper"),
        ]
        if self.level_progress > 2:
            new_enemies.append((random.randint(200, const.LEVEL_WIDTH - 200), random.randint(300, 500), "big"))
